...
export default function PocketApp() {
  ...
  const [modal, setModal] = useState<"item" | "invite" | "login" | "order" | null>(null);
    ...
    <div className="app-shell">
      ...
      <main className="main-shell">
        ...
        <div className={`page-area ${role === "customer" ? "customer-area" : ""}`}>
          ...
          {role === "customer" && screen === "profile" && <ProfileScreen notify={notify} onLogout={() => setModal("login")} />}
          ...
          {role !== "customer" && screen === "account" && <AccountScreen notify={notify} onLogout={() => setModal("login")} />}
        </div>
      </main>
  ...
  return (
    <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
      ...
      <nav className="side-nav">
        ...
        {navigation.map(({ id, label, icon: Icon, count }) => <button key={id} className={screen === id ? "active" : ""} onClick={() => onNavigate(id)}><Icon size={18} /><span>{label}</span>{count && <b>{count}</b>}</button>)}
      </nav>
...
function MenuManager({ venueName, onAdd, notify }: { venueName: string; onAdd: () => void; notify: (message: string) => void }) {
  ...
  return <><PageHeader title="Меню" subtitle="Управляйте категориями, ценами и доступностью блюд." actions={<><Button kind="secondary" icon={Eye} onClick={() => setPreviewOpen(true)}>Предпросмотр</Button><Button icon={Plus} onClick={onAdd}>Добавить позицию</Button></>} /><div className="menu-layout"><aside className="category-list"><div className="category-heading"><strong>Категории</strong><IconButton icon={Plus} label="Добавить категорию" onClick={() => setCategoryDialogOpen(true)} /></div><button className="mobile-category-trigger" onClick={() => setCategoryDialogOpen(true)}><Plus size={15} /><span>Добавить категорию</span></button>{categories.map((item) => <button className={category === item ? "active" : ""} key={item} onClick={() => setCategory(item)}><span>{item}</span><b>{counts[item] ?? 0}</b></button>)}</aside><section><div className="list-toolbar"><p><strong>{visible.length}</strong> позиций</p><div><label className="search-field"><Search size={17} /><input placeholder="Найти блюдо" /></label></div></div><div className="menu-view-row"><span>Вид меню</span><div className="view-toggle" aria-label="Вид меню"><IconButton icon={LayoutGrid} label="Показать сеткой" active={view === "grid"} onClick={() => setView("grid")} /><IconButton icon={Rows3} label="Показать списком" active={view === "list"} onClick={() => setView("list")} /></div></div><div className={`manager-grid view-${view}`}>{visible.map((item) => <article className="manager-item" key={item.id}><FoodImage item={item} />{item.popular && <span className="menu-status"><Sparkles size={12} />Хит</span>}<div className="manager-copy"><div><small>{item.category}</small></div><h3>{item.name}</h3><p>{item.description}</p><footer><strong>{money(item.price)}</strong><label className="switch"><input type="checkbox" defaultChecked onChange={() => notify("Доступность обновлена")} /><span /></label><IconButton icon={Pencil} label="Редактировать" onClick={onAdd} /><IconButton icon={Ellipsis} label="Действия с позицией" active={actionsItem === item.id} onClick={() => setActionsItem((current) => current === item.id ? null : item.id)} /></footer>{actionsItem === item.id && <div className="item-actions-menu"><button onClick={() => { setActionsItem(null); onAdd(); }}><Pencil size={16} />Редактировать</button><button onClick={() => runAction(`${item.name}: копия создана`)}><Copy size={16} />Дублировать</button><button onClick={() => runAction(`${item.name}: выберите новую категорию`)}><ArrowUpDown size={16} />Переместить в категорию</button><button onClick={() => runAction(`${item.name}: скрыто из меню`)}><EyeOff size={16} />Скрыть из меню</button><button className="danger" onClick={() => runAction(`${item.name}: удалено`)}><Trash2 size={16} />Удалить</button></div>}</div></article>)}</div></section></div>{categoryDialogOpen && <CategoryDialog value={categoryDraft} onChange={setCategoryDraft} onSubmit={addCategory} onClose={() => { setCategoryDialogOpen(false); setCategoryDraft(""); }} />}{previewOpen && <MenuPreview venueName={venueName} onClose={() => setPreviewOpen(false)} />}</>;
}
...
const subscriptionPlans = [
  { id: "start", name: "Start", price: 29, description: "Для небольшого заведения", venues: "1 заведение", features: ["Онлайн-меню и QR-коды", "Заказы и счета столов", "До 5 сотрудников"] },
  { id: "business", name: "Business", price: 69, description: "Для растущей команды", venues: "До 3 заведений", features: ["Все возможности Start", "Аналитика и роли команды", "Брони и предзаказы"] },
  { id: "pro", name: "Pro", price: 129, description: "Для сети заведений", venues: "Без ограничений", features: ["Все возможности Business", "Расширенная аналитика", "Приоритетная поддержка"] },
] as const;
...
function Modal({ type, onClose, notify }: { type: "item" | "invite" | "login" | "order"; onClose: () => void; notify: (message: string) => void }) {
  const copy = type === "item" ? { title: "Новая позиция", action: "Добавить в меню" } : type === "invite" ? { title: "Пригласить сотрудника", action: "Отправить приглашение" } : type === "login" ? { title: "Добро пожаловать", action: "Войти в Pocket" } : { title: "Заказ #1048", action: "Закрыть" };
  return <div className="modal-backdrop" onMouseDown={onClose}><section className={`modal ${type === "order" ? "order-modal" : ""}`} onMouseDown={(event) => event.stopPropagation()}><header><div><p className="eyebrow">{type === "login" ? "POCKET ACCOUNT" : "NORTH & VINE"}</p><h2>{copy.title}</h2></div><IconButton icon={X} label="Закрыть" onClick={onClose} /></header>{type === "item" && <div className="modal-body"><button className="image-drop"><ImagePlus size={24} /><strong>Добавить фотографию</strong><span>PNG или JPG до 8 МБ</span></button><div className="form-grid"><Field label="Название" wide><input placeholder="Название блюда" /></Field><Field label="Категория"><select><option>Закуски</option><option>Основное</option><option>Десерты</option></select></Field><Field label="Цена"><input placeholder="€ 0.00" /></Field><Field label="Описание" wide><textarea placeholder="Ингредиенты и короткое описание" /></Field></div></div>}{type === "invite" && <div className="modal-body"><div className="form-grid"><Field label="Имя"><input placeholder="Имя сотрудника" /></Field><Field label="E-mail"><input placeholder="name@example.com" /></Field><Field label="Роль" wide><select><option>Официант</option><option>Менеджер</option><option>Кухня</option><option>Только просмотр</option></select></Field></div><div className="permission-note"><ShieldCheck size={20} /><p><strong>Доступ по роли</strong><span>Сотрудник увидит только разрешенные разделы этого заведения.</span></p></div></div>}{type === "login" && <div className="modal-body auth-body"><div className="auth-logo"><span>P</span></div><p>Войдите, чтобы управлять заведением, бронировать столы и хранить историю заказов.</p><Field label="E-mail"><input defaultValue="denis@pocket.app" /></Field><Field label="Пароль"><input type="password" defaultValue="pocketdemo" /></Field><button className="forgot">Забыли пароль?</button><div className="auth-divider"><span>или</span></div><Button className="full" kind="secondary">Создать новый аккаунт</Button></div>}{type === "order" && <div className="modal-body order-detail"><div className="order-detail-top"><StatusPill status="Новый" /><span><Clock3 size={15} />2 минуты назад</span></div><div className="order-source"><span><Table2 size={20} /></span><div><strong>Стол 08 · 4 гостя</strong><p>Официант: София Марек</p></div></div><div className="order-lines"><p><b>2×</b><span>Тальятелле с трюфелем<small>Одна порция без пармезана</small></span><strong>€37.00</strong></p><p><b>1×</b><span>Буррата и томаты</span><strong>€13.50</strong></p><p><b>3×</b><span>Красный апельсин</span><strong>€21.00</strong></p></div><dl className="order-total"><div><dt>Подытог</dt><dd>€71.50</dd></div><div><dt>Чаевые</dt><dd>€5.00</dd></div><div><dt>Итого</dt><dd>€76.50</dd></div></dl></div>}<footer><Button kind="secondary" onClick={onClose}>{type === "login" ? "Отмена" : "Назад"}</Button><Button onClick={() => notify(type === "item" ? "Позиция добавлена в меню" : type === "invite" ? "Приглашение отправлено" : type === "login" ? "Вход выполнен" : "Заказ принят")}>{copy.action}</Button></footer></section></div>;
}
