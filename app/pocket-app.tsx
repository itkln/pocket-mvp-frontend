...
const initialFloors: VenueFloor[] = [{
  ...
  tables,
  fixtures: [
    { id: "bar-1", type: "bar", x: 18, y: 88 },
    { id: "window-1", type: "window", x: 94, y: 17 },
  ],
}];
...
const loadFloorPlan = (venueName: string) => {
  try {
    ...
    return floors.every((floor) => typeof floor.id === "string" && typeof floor.name === "string" && Array.isArray(floor.tables) && Array.isArray(floor.fixtures)) ? floors : initialFloors;
  } catch {
...
function FloorPlan({ mode, venueName, notify, embedded = false }: { mode: "owner" | "staff"; venueName: string; notify: (message: string) => void; embedded?: boolean }) {
  ...
  const addFixture = (type: FloorFixture["type"]) => {
    ...
    const fixture: FloorFixture = {
      id: `${type}-${Date.now()}`,
      type,
      x: type === "bar" ? 16 + (fixturesOfType.length % 2) * 44 : 6 + (fixturesOfType.length % 4) * 24,
      y: type === "bar" ? 86 - fixturesOfType.length * 10 : 5,
    };
    updateActiveFloor((floor) => ({ ...floor, fixtures: [...floor.fixtures, fixture] }));
    setSelected({ kind: "fixture", id: fixture.id });
    setAddMenuOpen(false);
    notify(`${type === "bar" ? "Бар добавлен" : "Окно добавлено"} на ${activeFloor.name.toLowerCase()}`);
  };
