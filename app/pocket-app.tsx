...
type FloorTable = { id: string; seats: number; x: number; y: number; state: "free" | "busy" | "reserved" };
type FloorFixture = { id: string; type: "bar" | "window" | "entrance"; x: number; y: number; rotation: number; size: number };
...
const initialFloors: VenueFloor[] = [{
  id: "floor-1",
  name: "1 этаж",
  tables,
  fixtures: [
...
const floorPlanKey = (venueName: string) => `pocket:floor-plan:${makeVenueSlug(venueName)}`;

const loadFloorPlan = (venueName: string) => {
  try {
...
const fixtureLabel = (type: FloorFixture["type"]) => type === "bar" ? "Бар" : type === "window" ? "Окно" : "Вход";

const downloadTableQr = async (venueName: string, tableId: string) => {
...
function OwnerTableEditor({ table, floorName, onSave, onDownload, onDelete }: { table: FloorTable; floorName: string; onSave: (id: string, seats: number) => void; onDownload: () => void; onDelete: () => void }) {
...
function StaffTableDetail({ table, floorName, notify }: { table: FloorTable; floorName: string; notify: (message: string) => void }) {
...
function QrCodesDialog({ venueName, tables: qrTables, onClose, notify }: { venueName: string; tables: FloorTable[]; onClose: () => void; notify: (message: string) => void }) {
