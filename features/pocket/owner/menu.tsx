...
export function MenuManager({ venueName, onAdd, notify }: { venueName: string; onAdd: () => void; notify: (message: string) => void }) {
  ...
  return <><PageHeader title="Меню" subtitle="Категории, цены и доступность блюд." actions={<><Button kind="secondary" icon={Eye} onClick={() => setPreview(true)}>Предпросмотр</Button><div className="menu-create-menu"><Button icon={Plus} onClick={() => setCreateOpen((value) => !value)}>Добавить</Button>{createOpen && <CreateMenu onItem={() => { setCreateOpen(false); onAdd(); }} onCategory={() => { setCreateOpen(false); setCategoryDialog(true); }} />}</div></>} />
    ...
    {categoriesOpen && <CategoryManagerDialog categories={workspace.categories} onClose={() => setCategoriesOpen(false)} onCreate={() => { setCategoriesOpen(false); setCategoryDialog(true); }} onRemove={removeCategory} onSave={async (categories) => { await Promise.all(categories.map((item) => workspace.editCategory(item.id, { name: item.name, sort_order: item.sort_order, is_active: item.is_active }))); setCategoriesOpen(false); notify("Категории сохранены"); }} />}
...
function CategoryManagerDialog({ categories, onClose, onCreate, onRemove, onSave }: { categories: OwnerCategory[]; onClose: () => void; onCreate: () => void; onRemove: (category: OwnerCategory) => Promise<boolean>; onSave: (categories: OwnerCategory[]) => Promise<void> }) {
  const [drafts, setDrafts] = useState(() => categories.map((category) => ({ ...category })));
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      await onSave(changed.map((category) => ({ ...category, name: category.name.trim() })));
    } finally {
      setSaving(false);
    }
  };

  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal category-manager-dialog" onMouseDown={(event) => event.stopPropagation()}><header><h2>Управление категориями</h2><IconButton icon={X} label="Закрыть" onClick={onClose} /></header><form onSubmit={submit}><div className="modal-body">{drafts.length ? <div className="category-manager-list">{drafts.map((category) => <div className="category-manager-row" key={category.id}><div><input aria-label={`Название категории ${category.name}`} value={category.name} onChange={(event) => setDrafts((current) => current.map((item) => item.id === category.id ? { ...item, name: event.target.value } : item))} /><small>{positionCount(category.item_count)}</small></div><label className="category-visibility"><span>{category.is_active ? "В меню" : "Скрыта"}</span><span className="switch"><input type="checkbox" aria-label={`${category.is_active ? "Скрыть" : "Опубликовать"} категорию ${category.name}`} checked={category.is_active} onChange={(event) => setDrafts((current) => current.map((item) => item.id === category.id ? { ...item, is_active: event.target.checked } : item))} /><span /></span></label><button type="button" className="category-delete" disabled={category.item_count > 0} aria-label={`Удалить категорию ${category.name}`} title={category.item_count > 0 ? "Сначала удалите или перенесите позиции" : "Удалить категорию"} onClick={() => void onRemove(category).then((removed) => { if (removed) setDrafts((current) => current.filter((item) => item.id !== category.id)); })}><Trash2 size={17} /></button></div>)}</div> : <div className="category-manager-empty">Категорий пока нет</div>}</div><footer><Button kind="secondary" icon={Plus} onClick={onCreate}>Новая категория</Button><div><Button kind="secondary" onClick={onClose}>Закрыть</Button><Button type="submit" disabled={!changed.length || invalid || saving}>{saving ? "Сохраняем..." : "Сохранить"}</Button></div></footer></form></section></div>;
}
