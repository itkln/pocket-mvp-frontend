...
export function MenuManager({ venueName, onAdd, notify }: { venueName: string; onAdd: () => void; notify: (message: string) => void }) {
  ...
  const editItem = (item: OwnerMenuItem) => {
    setSelectedItemID(item.id);
    setActionItem(null);
    if (window.matchMedia("(min-width: 1181px)").matches) setDetailEditItemID(item.id);
...
function SortableMenuCard({ item, selected, reorderEnabled, actionOpen, onSelect, onToggleActions, onEdit, onToggleAvailable, onTogglePopular, onDuplicate, onRemove }: { item: OwnerMenuItem; selected: boolean; reorderEnabled: boolean; actionOpen: boolean; onSelect: () => void; onToggleActions: () => void; onEdit: () => void; onToggleAvailable: () => void; onTogglePopular: () => void; onDuplicate: () => void; onRemove: () => void }) {
  ...
  const style: CSSProperties = { transform: CSS.Transform.toString(transform), transition };
  return <article ref={setNodeRef} style={style} className={`menu-product-row ${selected ? "selected" : ""} ${isDragging ? "dragging" : ""}`} onClick={onSelect}>{item.image_url ? <div className="food-image" style={{ backgroundImage: 'url("' + item.image_url + '")' }} role="img" aria-label={item.name} /> : <div className="food-image menu-image-placeholder"><span>{item.name.at(0)}</span></div>}<div className="menu-product-copy"><div><h3>{item.name}</h3>{item.is_popular && <Star size={13} />}</div><p>{money(item.price_minor / 100, item.currency)}</p></div><label className="switch menu-product-switch" title={item.is_available ? "Скрыть позицию" : "Опубликовать позицию"} onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={item.is_available} onChange={onToggleAvailable} /><span /></label><div className="row-actions" onClick={(event) => event.stopPropagation()}><IconButton icon={Ellipsis} label="Действия с позицией" onClick={onToggleActions} />{actionOpen && <div className="row-actions-menu"><button onClick={onEdit}><Pencil size={16} />Редактировать</button><button onClick={onTogglePopular}><Star size={16} />{item.is_popular ? "Убрать «Хит»" : "Отметить как «Хит»"}</button><button onClick={onDuplicate}><Copy size={16} />Создать копию</button><button className="danger" onClick={onRemove}><Trash2 size={16} />Удалить</button></div>}</div>{reorderEnabled && <button type="button" className="menu-product-drag" aria-label={t("Переместить позицию {name}", { name: item.name })} {...attributes} {...listeners}><GripVertical size={17} /></button>}</article>;
}

function MenuItemDetails({ item, categories, startEditing, onCancelEdit, onSave, onToggleAvailable, onTogglePopular }: { item: OwnerMenuItem; categories: OwnerCategory[]; startEditing: boolean; onCancelEdit: () => void; onSave: (input: MenuItemInput) => Promise<void>; onToggleAvailable: () => void; onTogglePopular: () => void }) {
  const [editing, setEditing] = useState(startEditing);
  const [saving, setSaving] = useState(false);
  const [draft, setDraft] = useState({ categoryID: item.category_id, name: item.name, description: item.description ?? "", price: (item.price_minor / 100).toFixed(2), imageURL: item.image_url ?? "" });
  const cancel = () => { setEditing(false); setDraft({ categoryID: item.category_id, name: item.name, description: item.description ?? "", price: (item.price_minor / 100).toFixed(2), imageURL: item.image_url ?? "" }); onCancelEdit(); };
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setSaving(true);
    try {
      await onSave(itemInput(item, { category_id: draft.categoryID, name: draft.name.trim(), description: draft.description.trim(), price_minor: Math.round(Number(draft.price) * 100), image_url: draft.imageURL }));
      setEditing(false);
    } finally {
      setSaving(false);
    }
  };
  return <div className="menu-item-details"><div className="menu-detail-media">{editing ? <MenuImageField value={draft.imageURL} name={draft.name} onChange={(imageURL) => setDraft((current) => ({ ...current, imageURL }))} /> : <>{item.image_url ? <div className="food-image" style={{ backgroundImage: 'url("' + item.image_url + '")' }} role="img" aria-label={item.name} /> : <div className="food-image menu-image-placeholder"><span>{item.name.at(0)}</span></div>}{item.is_popular && <span className="menu-status"><Star size={12} />Хит</span>}</>}</div>{editing ? <form className="menu-inline-editor" onSubmit={submit}><div className="menu-inline-grid"><label className="field"><span>Название</span><input autoFocus value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} required /></label><label className="field"><span>Категория</span><select value={draft.categoryID} onChange={(event) => setDraft((current) => ({ ...current, categoryID: event.target.value }))}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label className="field"><span>Цена</span><input type="number" min="0" step="0.01" value={draft.price} onChange={(event) => setDraft((current) => ({ ...current, price: event.target.value }))} required /></label><label className="field wide"><span>Описание</span><textarea value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} /></label></div><footer><Button kind="quiet" onClick={cancel}>Отмена</Button><Button type="submit" disabled={saving || !draft.name.trim()}>{saving ? "Сохраняем..." : "Сохранить"}</Button></footer></form> : <><header><div><small>{item.category}</small><h2>{item.name}</h2></div><IconButton icon={Pencil} label="Редактировать позицию" onClick={() => setEditing(true)} /></header><section className="menu-detail-description"><span>Описание</span><p>{item.description || "Описание не добавлено"}</p></section><dl className="menu-detail-values"><div><dt>Цена</dt><dd>{money(item.price_minor / 100, item.currency)}</dd></div><div><dt>Статус</dt><dd>{item.is_available ? "Доступна гостям" : "Скрыта из меню"}</dd></div></dl></>}<div className="menu-detail-setting"><div><strong>Доступность</strong><p>{item.is_available ? "Позиция отображается в меню" : "Позиция временно скрыта"}</p></div><label className="switch"><input type="checkbox" checked={item.is_available} onChange={onToggleAvailable} /><span /></label></div><label className={`menu-highlight-setting ${item.is_popular ? "active" : ""}`}><span className="menu-highlight-icon"><Star size={18} /></span><div><strong>Отметка «Хит»</strong><p>Показывать заметную метку в меню</p></div><span className="switch"><input type="checkbox" checked={item.is_popular} onChange={onTogglePopular} /><span /></span></label></div>;
}
