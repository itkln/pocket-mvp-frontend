...
export function Modal({ type, onClose, notify }: { type: "item" | "invite" | "order"; onClose: () => void; notify: (message: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className={`modal ${type === "order" ? "order-modal" : ""}`} onMouseDown={(event) => event.stopPropagation()}><header><div><h2>{title}</h2></div><IconButton icon={X} label="Закрыть" onClick={onClose} /></header>
    ...
    </form> : <><div className="modal-body order-detail">{order ? <><div className="order-detail-top"><StatusPill status={order.status} /><span><Clock3 size={15} />{new Date(order.created_at).toLocaleString("ru-RU")}</span></div><div className="order-source"><span><Table2 size={20} /></span><div><strong>{order.source}</strong><p>{order.guest_name || "Гость"}</p></div></div><dl className="order-total"><div><dt>Итого</dt><dd>{money(order.total_minor / 100)}</dd></div></dl></> : <p>Заказов пока нет.</p>}</div><footer><Button kind="secondary" onClick={onClose}>Закрыть</Button>{order && order.status === "new" && <Button onClick={async()=>{await workspace.setOrderStatus(order.id,"accepted");notify("Заказ принят");onClose();}}>Принять заказ</Button>}</footer></>}
  </section></div>;
}
